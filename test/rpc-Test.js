...
test('Building empty JSon Rpc Request', function() {
	var req1 = wsrpc.RPC.Request('id1', 'action1', null);
	var req2 = wsrpc.RPC.Request('id2', 'action2');
	assert.equal(req1.jsonrpc, wsrpc.RPC.VERSION);
	assert.equal(req1.id, 'id1');
	assert.equal(req1.method, 'action1');
	assert.equal(req1.params, null);
    assert.equal(req2.jsonrpc, wsrpc.RPC.VERSION);
...
test('Building empty JSon Rpc Response', function() {
    var req1 = wsrpc.RPC.Response('id1');
    var req2 = wsrpc.RPC.Response('id2', null);
    assert.equal(req1.jsonrpc, wsrpc.RPC.VERSION);
    assert.equal(req1.id, 'id1');
...
test('Building error JSon Rpc', function() {
    var err1 = wsrpc.RPC.Error('id1',
        wsrpc.RPC.PARSE_ERROR,
        'Parse error message'
    );
    assert.equal(err1.jsonrpc, wsrpc.RPC.VERSION);
...
test('Parse JSon Rpc Request', function() {
    ...
    var parser1 = new wsrpc.RPCParser(req1);
    ...
    var parser2 = new wsrpc.RPCParser(req2);
...
test('Parse JSon Rpc Response', function() {
    ...
    var parser = new wsrpc.RPCParser(resp);
...
test('Parse JSon Rpc Error', function() {
    ...
    var parser = new wsrpc.RPCParser(error);
